...
export const listProperties = () => async (dispatch,getState) => {
    ...
    try {    
      const { data } = await axios.get(`${URL}/api/users/get-properties`,{
  ...
  export const addproperty = (name,location,address,map,description,starring,restaurants,bars,holiday_type,checkin,checkout,tax,discount,extra_charges,cancellation_policy,t_and_c,transfer,meals) =>async(dispatch,getState)=>{
    dispatch({type:ADD_PROPERTIES_REQUEST,payload:name,location,address,map,description,starring,restaurants,bars,holiday_type,checkin,checkout,tax,discount,extra_charges,cancellation_policy,t_and_c,transfer,meals});
    const {
      userSignin: { userInfo },
    } = getState();

    try{
        const {data} =await axios.post(`${URL}/api/users/add-property`,{name,location,address,map,description,starring,restaurants,bars,holiday_type,checkin,checkout,tax,discount,extra_charges,cancellation_policy,t_and_c,transfer,meals},{
          headers: { "x-auth-token": `${userInfo.token}` },
        })
...
export const deleteProperty = (propertytId) => async (dispatch,getState) => {
  ...
  try {
    await axios.delete(`${URL}/api/users/delete-property/${propertytId}`,{
      headers: { "x-auth-token": `${userInfo.token}` },
    })
...
export const detailsProperty = (propertyId) => async (dispatch,getState) => {
  ...
  try {
    const { data } = await axios.get(`${URL}/api/users/get-property/${propertyId}`,{
      headers: { "x-auth-token": `${userInfo.token}` },
    });
...
export const addRoom = (name,description,adult,child,size,bedType,amenities,first,second,third,fourth,count,propId) =>async(dispatch,getState)=>{
    dispatch({type:ADD_ROOM_REQUEST,payload:name,description,adult,child,size,bedType,amenities,price:{first,second,third,fourth},count,propId});

    const {
      userSignin: { userInfo },
    } = getState();
    try{
        const {data} = await axios.post(`${URL}/api/users/add-room`,{name,description,adult,child,size,bedType,amenities,price:{first,second,third,fourth},count,propId},{
          headers: { "x-auth-token": `${userInfo.token}` },
        })
...
export const getRoomsDetails = (propertyId) => async (dispatch,getState) => {
  ...
  try {
    const { data } = await axios.get(`${URL}/api/users/get-rooms/${propertyId}`,{
      headers: { "x-auth-token": `${userInfo.token}` },
    });
...
export const detailsRoom = (propId,roomId) => async(dispatch,getState)=>{
  ...
  try{
     const {data} = await axios.get(`${URL}/api/users/get-a-room/id?propId=${propId}&roomId=${roomId}`,{
      headers: { "x-auth-token": ` ${userInfo.token}` },
     })
  ...
  export const deleteRooms = (propId,roomId) => async (dispatch,getState) => {
    ...
    try {
    const {data}=  await axios.delete(`${URL}/api/users/delete-room/id?propId=${propId}&roomId=${roomId}`,{
      headers: { "x-auth-token": ` ${userInfo.token}` },
    })
  ...
  export const updateRoom=(room)=>async(dispatch,getState)=>{
    ...
    try{
      const {data} = await axios.put(`${URL}/api/users/update-room/`,room,{
        headers: { "x-auth-token": ` ${userInfo.token}` },
      })
  ...
  export const checkProperty = (location,adult,child,from,to) =>async(dispatch)=>{
    ...
    try{
        const {data} =await axios.post(`${URL}/api/clients/check-availability`,{location,adult,child,from,to},)
...
export const confirmBooking = (propId,roomId,name,email,code,phone,country,from,to,price) =>async(dispatch)=>{
  ...
  try{
      const {data} =await axios.post(`${URL}/api/clients/confirm-booking`,{propId,roomId,name,email,phone:code+phone,country,from,to,price})
...
export const createAccount=( f_name,l_name,email,phone,address)=>async(dispatch)=>{
   ...
   try{
     const {data}=await axios.post(`${URL}/api/users/signup`,{ f_name,l_name,email,phone,address})
...
export const deletePropImg=(propId,imageId)=>async(dispatch)=>{
  ...
  try {
  const {data}=await axios.post(`${URL}/api/users/property-delete`,propId,imageId)  
...
export const deleteRoomImg = (propId,roomId,imageId) => async (dispatch) => {
  ...
  try {
  const {data}=  await axios.delete(`${URL}/api/users/room-delete`,propId,roomId,imageId)   
