...
export const getCountDetails = () => async (dispatch,getState) => {
  ...
  export const getSignUpRequestAgent=()=>async(dispatch,getState)=>{
    dispatch({type:AGENT_GET_SIGNUP_REQUEST})
    const {
      adminSignin: { adminInfo},
    } = getState(); 
    
    try{
      const {data}= await axios.get(`${URL}/api/admin/get-agent-requests`,{
        headers: { "x-auth-token": `${adminInfo.token}` }, 
      })
      dispatch({type:AGENT_GET_SIGNUP_SUCCESS,payload:data})
    }catch(error){
      const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
       dispatch({ type: AGENT_GET_SIGNUP_FAIL, payload: message });
    
    }
  }


  export const approveAgent=(userId)=>async(dispatch,getState)=>{
    dispatch({type:APPROVE_AGENT_REQUEST});
    const {adminSignin: { adminInfo },} = getState();

    try{
      const {data} = await axios.put(`${URL}/api/admin/approve-agent/${userId}`,)
 
      dispatch({type:APPROVE_AGENT_SUCCESS,payload:data});
      ...
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
       dispatch({ type: APPROVE_AGENT_FAIL, payload: message });
    }
  }

  export const rejectAgent=(userId)=>async(dispatch,getState)=>{
    dispatch({type:REJECT_AGENT_REQUEST});
    try{
      const {data} = await axios.put(`${URL}/api/admin/reject-user/${userId}`)
      dispatch({type:REJECT_AGENT_SUCCESS,payload:data});
      ...
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
       dispatch({ type: REJECT_AGENT_FAIL, payload: message });
    }
  }


  export const getBookingsAgent = () => async (dispatch,getState) => {
    dispatch({ type: AGENT_BOOKING_DETAILS_REQUEST });
    const {
      adminSignin: { adminInfo },
    } = getState();
    try {
      const { data } = await axios.get(`${URL}/api/admin/get-all-agentBooking`);
      dispatch({ type: AGENT_BOOKING_DETAILS_SUCCESS, payload: data });
    } catch (error) {
      dispatch({
        type: AGENT_BOOKING_DETAILS_FAIL,
        payload:
          error.response && error.response.data.message
            ? error.response.data.message
            : error.message,
  ...
  export const approveBookingAgent=(bookId)=>async(dispatch,getState)=>{
    dispatch({type:APPROVE_AGENT_BOOKING_REQUEST});
    const {
      adminSignin: { adminInfo },
    } = getState();
    
    try{
      const {data} = await axios.put(`${URL}/api/admin/approve-agentBooking/${bookId}`)
      dispatch({type:APPROVE_AGENT_BOOKING_SUCCESS,payload:data});
      ...
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
       dispatch({ type: APPROVE_AGENT_BOOKING_FAIL, payload: message });
    }
  }

  export const rejectBookingAgent=(bookId)=>async(dispatch,getState)=>{
    dispatch({type:REJECT_AGENT_BOOKING_REQUEST});
    const {
      adminSignin: { adminInfo },
    } = getState();
    try{
      const {data} = await axios.put(`${URL}/api/admin/reject-agentBooking/${bookId}`)
      dispatch({type:REJECT_AGENT_BOOKING_SUCCESS,payload:data});
      ...
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
       dispatch({ type: REJECT_AGENT_BOOKING_FAIL,payload: message });
    }
  }