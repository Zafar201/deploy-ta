...
function Criteria2({destination,startingDate,endingDate,holidayType,adultCount,childCount,Nights,selectedDate,selectedDate2,nationalities,childAge}) {
    
      const [show, setShow] = useState(false);

      const handleClose = () => setShow(false);
      const handleShow = () => setShow(true);
      console.log(childAge,'hs');
  return (
    <div>
   ...
   <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body className='model' >
          <SearchCriteria handleClose={handleClose} show={show} 
          to={selectedDate2}
          from={selectedDate}
          Nights={Nights} 
                          ...
                          {adultCount && (
                            <>
                             ...
                             <p >{adultCount} x adults</p>
                              {childCount > 0 && (<p className={`${childAge.length > 0 ? 'p-bt' : ''}`}>{childCount} x children ({childAge.length > 0 && childAge.map((a)=>(
                                <span>{a}yr</span>
                              ))} )</p>)}
                             </>
                          )}
